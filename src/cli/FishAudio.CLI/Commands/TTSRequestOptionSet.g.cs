...
internal sealed record TTSRequestOptionSet(
    Option<string> Text,
                     Option<double?> Temperature,
                     Option<double?> TopP,
                     Option<int?> ChunkLength,
                     Option<bool?> Normalize,
                     Option<global::FishAudio.TTSRequestFormat?> Format,
                     Option<int?> SampleRate,
                     Option<int?> Mp3Bitrate,
                     Option<int?> OpusBitrate,
                     Option<global::FishAudio.TTSRequestLatency?> Latency,
...
{
    public static TTSRequestOptionSet Create(string? prefix = null)
    {
        var normalizedPrefix = string.IsNullOrWhiteSpace(prefix)
            ? string.Empty
            : prefix.Trim().Trim('-') + "-";
        return new TTSRequestOptionSet(
                ...
                Format: new Option<global::FishAudio.TTSRequestFormat?>($"--{normalizedPrefix}format")
                {
                ...
                Latency: new Option<global::FishAudio.TTSRequestLatency?>($"--{normalizedPrefix}latency")
                {
